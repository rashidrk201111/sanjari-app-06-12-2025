...
DECLARE r RECORD;
BEGIN
    FOR r IN (SELECT policyname FROM pg_policies WHERE tablename = 'payment_gateway_settings') LOOP
        EXECUTE 'DROP POLICY IF EXISTS "' || r.policyname || '" ON public.payment_gateway_settings';
    END LOOP;
END $$;
...
CREATE POLICY "payment_all_if_admin" ON public.payment_gateway_settings
    FOR ALL 
    USING (public.is_admin());
...
-- ============================================
-- ✅ COMPLETE! All recursion errors fixed!
-- ============================================
-- Now test your app:
